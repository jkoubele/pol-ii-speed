...
usage() {
    echo "Usage: $0 -i <input_folder> -o <output_folder> -b <bam_folder>"
    exit 1
}
...
if [ -z "$input_folder" ] || [ -z "$output_folder" ] || [ -z "$bam_folder" ]; then
...
for sub_folder in "$input_folder"/*; do
  ...
  "$repository_path"/jobs/remove_intronic_reads_from_fastq.sh \
  -i "$sub_folder" \
  -o "$output_folder"/"$sample_name" \
  -b "$bam_folder"/"$sample_name" \
  -s "$script_folder"
done
