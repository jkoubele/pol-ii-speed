...
class CoverageData(NamedTuple):
    ...
    intron_coverages: dict[tuple[str, int], IntronCoverage]


def load_coverage_data() -> CoverageData:
    ...
    for intron_name in intron_names:
        for sample_id, _ in enumerate(design_matrix):
            ...
            intron_coverage = IntronCoverage(coverage=coverage_vector)
            intron_coverages[(intron_name, sample_id)] = intron_coverage
...
logits: dict[tuple[str, int], cp.Expression] = {}
