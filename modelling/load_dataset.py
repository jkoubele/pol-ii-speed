...
from pol_ii_model import GeneData, DatasetMetadata
...
gene_file = 'test_subset.csv'
gene_file = 'protein_coding_genes.csv'


design_matrix_df = pd.read_csv(results_folder / 'experiment_design' / 'design_matrix.csv')

library_size_factors_df = pd.read_csv(results_folder / 'aggregated_counts' / 'library_size_factors.tsv',
                                      sep='\t')
...
gene_names_df = pd.read_csv(results_folder / 'gene_names' / gene_file)

exon_counts_df = pd.read_csv(results_folder / 'aggregated_counts' / 'exon_counts.tsv',
                             sep='\t').set_index('gene_id')
exon_counts_df = exon_counts_df[dataset_metadata.sample_names]

intron_counts_df = pd.read_csv(results_folder / 'aggregated_counts' / 'intron_counts.tsv',
                             sep='\t').set_index('intron_id')
intron_counts_df = intron_counts_df[dataset_metadata.sample_names]
...
for sample_name in tqdm(dataset_metadata.sample_names, desc='Loading coverage data'):
    coverage_df = pd.read_parquet(results_folder / 'rescaled_coverage' / f'{sample_name}.parquet')
...
for gene_id in tqdm(gene_names_df['gene_id'].unique(), desc = 'Preparing gene data'):
    ...
    exon_reads = torch.tensor(exon_row, dtype = torch.float32)
    intron_reads = torch.tensor(intron_counts_df.loc[introns_to_keep].values, dtype=torch.float32).T
    
    coverage_of_bases = torch.stack([
        torch.tensor([coverage_df_by_sample[sample].loc[intron] for intron in introns_to_keep], dtype=torch.float32)
        for sample in dataset_metadata.sample_names
    ])
